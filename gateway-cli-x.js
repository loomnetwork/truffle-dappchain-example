...
const MyRinkebyERC721XJSON = require('./src/contracts/MyERC721XRinkebyToken.json')
const MyERC721XJSON = require('./src/contracts/MyERC721XToken.json')
...
async function withdrawEthFromRinkebyGateway({
  web3js,
  amount,
  accountAddress,
  signature,
  ...
  .action(async function(contractType, options) {
    ...
    try {
      ...
      if (contractType === 'coin') {
        ...
        tokenRinkebyAddress = MyRinkebyERC721XJSON.networks[rinkebyNetworkId].address
        rinkebyTxHash = MyRinkebyERC721XJSON.networks[rinkebyNetworkId].transactionHash
        tokenExtdevAddress = MyERC721XJSON.networks[extdevNetworkId].address
      } else {
